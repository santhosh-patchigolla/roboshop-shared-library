    ...
    pipeline {
        ...
        stages {
            ...
            stage('Verify the artifact version') {
                ...
                steps {
                    script {
                        env.UPLOAD_STATUS=sh (returnStdout: true, script: 'curl -L -s  http://172.31.93.234:8081/service/rest/repository/browse/${COMPONENT}/ | grep ${COMPONENT}-${TAG_NAME} || true' )    // true is used for the If the first statement is failed will treat as true
                        print UPLOAD_STATUS                     
                    }
                }
            }
